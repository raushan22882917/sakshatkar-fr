...
import { Star, Users, Briefcase, GraduationCap, Clock } from 'lucide-react';
...
export function MentorDetails() {
  ...
  // Fetch the mentor details based on the id
  const { data: mentor, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        `)
        .eq('id', id) // Fetch mentor data where the id matches the URL parameter
        .single();
  ...
  if (error || !mentor) return <div>Mentor not found</div>;
                    ...
                    <div className="space-y-4">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-gray-600">{mentor.company || 'Independent Mentor'}</p>
                        </div>
                      </div>
                      <div className="flex items-start space-x-3">
