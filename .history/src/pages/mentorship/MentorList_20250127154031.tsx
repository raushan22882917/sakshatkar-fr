...
import { Users, Clock, Search, Star } from 'lucide-react';
  ...
  const { data: mentors, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`*, profiles:user_id (name, email, company_name)`);
        ...
        <div className="text-center mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
            <div className="border rounded-lg shadow-lg p-4 flex flex-col items-center">
              ...
              <h3 className="text-xl font-bold mb-2">Search Mentors</h3>
              <p className="text-gray-600 dark:text-gray-400">Find mentors based on expertise.</p>
            </div>
            <div className="border rounded-lg shadow-lg p-4 flex flex-col items-center">
              <Users className="w-12 h-12 text-blue-500 mb-4" />
              <h3 className="text-xl font-bold mb-2">Join a Community</h3>
              <p className="text-gray-600 dark:text-gray-400">Learn from experienced mentors.</p>
            </div>
            <div className="border rounded-lg shadow-lg p-4 flex flex-col items-center">
              <Clock className="w-12 h-12 text-blue-500 mb-4" />
              <h3 className="text-xl font-bold mb-2">Flexible Sessions</h3>
              <p className="text-gray-600 dark:text-gray-400">Schedule sessions at your convenience.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              ...
              filteredMentors?.map((mentor) => (
                <Card key={mentor.id} className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate(`/mentorship/${mentor.id}`)}>
                  <CardHeader className="flex flex-row items-center space-x-4">
                    <img src={mentor.profile_image_url || '/placeholder.svg'} alt={mentor.profiles?.name || 'Mentor'} className="w-16 h-16 rounded-full object-cover border-2 border-purple-500" />
                    <div>
                  ...
                  <CardContent>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{mentor.bio?.substring(0, 150) || 'No bio available.'}</p>
