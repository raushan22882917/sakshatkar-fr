...
<div className="flex flex-col items-center space-y-6 p-4">
  ...
  {/* Progress and Timer */}
  <div className="flex items-center justify-between py-4 w-full max-w-[900px]">
    <div className="flex items-center space-x-2">
      <Timer className="h-5 w-5 text-purple-500" />
      <span className="font-mono text-xl">{formatTime(timeLeft)}</span>
    </div>
    <div className="flex-1 mx-4">
      <Progress value={progress} className="h-2 rounded-full bg-purple-300" />
    </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-[900px]">
    ...
    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg overflow-hidden">
      <CardHeader className="bg-purple-500 p-4 rounded-t-lg">
        <CardTitle className="flex items-center gap-2 text-xl font-semibold text-white">
          <Brain className="h-5 w-5 text-white" />
          {currentQuestion.type.toUpperCase()} Question
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4 bg-white text-lg text-gray-700">
    ...
    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg overflow-hidden">
      <CardHeader className="bg-purple-500 p-4 rounded-t-lg">
        <CardTitle className="flex justify-between items-center text-white">
          ...
          {currentQuestion.type === 'code' && (
            <Select value={language} onValueChange={handleLanguageChange} className="bg-white rounded-lg">
      ...
      <CardContent className="p-4 bg-white">
        {currentQuestion.type === 'code' ? (
          ...
          <textarea
            className="w-full h-[300px] p-4 rounded-lg border resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 bg-transparent text-black dark:text-white placeholder-gray-400"
            value={answer}
  ...
  <div className="flex justify-between py-4 w-full max-w-[900px]">
    <Button variant="destructive" onClick={() => setIsStarted(false)} className="bg-red-500 text-white">
      <StopCircle className="mr-2 h-4 w-4" />
      End Interview
    </Button>
    <div className="space-x-2">
      <Button variant="outline" onClick={handleSubmitAnswer} className="bg-purple-500 text-white">
